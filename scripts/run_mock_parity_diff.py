...
def main() -> int:
    ...
    manifest = load_manifest(repo_root / "mock_parity_scenarios.json")
    parity_text = load_parity_text(repo_root / "docs" / "RUST_PARITY.md")

    missing_refs = ensure_refs_exist(manifest, parity_text)
    if missing_refs:
        print("Missing docs/RUST_PARITY.md references:", file=sys.stderr)
        for scenario_name, ref in missing_refs:
            print(f"  - {scenario_name}: {ref}", file=sys.stderr)
        return 1

    should_run = "--no-run" not in sys.argv[1:]
    report = run_harness(repo_root) if should_run else None
    report_by_name = (
    ...
    print(f"Scenario manifest: {repo_root / 'mock_parity_scenarios.json'}")
    print(f"PARITY source: {repo_root / 'docs' / 'RUST_PARITY.md'}")
    print()
